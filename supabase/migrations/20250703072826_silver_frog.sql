/*
  ...
  1. New Tables
    - `users` - User profiles with authentication integration
      ...
      - `email` (text, unique)
      - `name` (text)
      - `role` (text, constrained to 'student' or 'teacher')
    ...
    - `quizzes` - Quiz management
      ...
      - `questions` (jsonb, default empty array)
      - `created_at` (timestamptz, default now)
    ...
    - `quiz_attempts` - Student quiz submissions
      ...
      - `answers` (jsonb, default empty object)
    ...
    - `subjective_tests` - Essay/subjective assessments
      - `id` (uuid, primary key)
      - `created_by` (uuid, foreign key to users)
      - `title` (text)
      - `document_id` (uuid, foreign key to documents)
    ...
    - `subjective_submissions` - Student subjective test submissions
      ...
      - `user_id` (uuid, foreign key to users)
      - `answers` (jsonb, default empty array)
      - `ai_score` (double precision, default 0)
      - `teacher_score` (double precision, nullable)
      - `ai_feedback` (text, nullable)
      - `teacher_feedback` (text, nullable)
      - `status` (text, default 'submitted', constrained)
      - `submitted_at` (timestamptz, default now)
      - `ai_graded_at` (timestamptz, nullable)
      - `teacher_reviewed_at` (timestamptz, nullable)

  2. Security
    - Enable RLS on all tables
    - Add role-based policies for students and teachers
    - Ensure users can only access their own data or appropriate shared data
